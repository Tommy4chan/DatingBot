...
from bot.database.methods.select import get_new_users_count_by_data, get_active_users_count_by_data, is_user_admin, get_user_count, get_fake_users_count
...
async def __stats(message: types.Message):
    ...
    if await is_user_admin(chat_id) and len(date) != 0:
        ...
        fake_users = await get_fake_users_count()

        await bot.send_message(chat_id, f'Дата: {date}\n\nНових користувачів: {new_users}\nАктивних користувачів: {active_users}\nЗагалом фейкових користувачів: {fake_users}\nЗагалом реальних користувачів: {total_users}')
